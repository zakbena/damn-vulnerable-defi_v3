...
describe("[Challenge] Free Rider", function () {
  ...
  it("Execution", async function () {
    ...
    let recoveryContractFactory = await ethers.getContractFactory(
      "Recover",
      player
    );

    let pair = await uniswapFactory.getPair(token.address, weth.address);
    let recover = await recoveryContractFactory.deploy(
      marketplace.address,
    ...
    let recoverTX = await recover.connect(player).recoverNFT();
  });
